...
10. **スマホ対応(Unity版のみ、2026-09-11に着手)**。ユーザーの選択: (1) 指で塊と測定円が隠れるので、**タッチのときだけ引き寄せる点(=カーソル)を指より上にずらす**。まずCSSで60px(約1cm)から始めて試遊で調整する。ミュートボタンとスライダーの判定は指そのものの位置で行う / (2) **横向き前提**。縦向きのときは画面全体に「横向きにしてください」を出し、その間は入力を受け付けない(長押し中に縦にしたら、離したのと同じく確定) / (3) **実機確認はAndroidのChrome**。iPhoneは確認できないので、iOS特有の挙動は仕様に沿った対処だけ入れる。物理・盤面(1920×1080)・PCのマウス操作は変えない。ランキングはPCと共通(ずらしで有利不利が出ないかは試遊で見る)。Brainの調べで分かっていた前提: unityroomは`Build/`の4ファイルしか使わないので、テンプレートのCSSは効かず、長押しの文字選択やスクロールの抑止はビルド内のjspreで行う / タッチの`pointerdown`はブラウザのユーザー操作に数えられない(HTML仕様。タッチは`pointerup`/`touchend`)ので、音の開始は離したときにも行う。**進捗**: 実装済み(C#はagy、jslib・jspreはBrain)。ヘッドレスChromeのタッチ再現と、Androidの実機(ユーザー)で確認済みで、ずらす量60 CSS pxは実機で問題なし。実機で見つかった「アドレスバーに画面の下が隠れる」は手元用テンプレートの`100vh`が原因で、直した。unityroomに投稿してユーザーが確認したところ、(a) 縦向きのまま遊べた(unityroomのページはスマホを縦にしてもキャンバスを横長で置くので、判定が働かない) (b) 横にすると画質が落ちた(unityroomのページがスマホでは描画解像度を1倍に固定する)。**ユーザー判断で(2)の縦向きの判定と案内は外した**(縦向きでは小さい盤面のまま遊べる)。(b)は`pixel-ratio.jspre`で端末の解像度(上限3)に戻す(`docs/dev-notes.md`)。修正版をunityroomへ出し直し、ユーザーがスマホで動作と表示を確認した(2026-09-11)。
11. **描画の補間(Unity版のみ、2026-09-16)**。165Hz表示では物理が固定1/60秒のため動きが60Hzに量子化されていた(`docs/handoff.md` §5-6)。物理・`FIXED_DT`・golden基準データは変えず、描画側だけで対処する: `Simulation/RenderInterpolator.cs`(純粋C#、`Span`ベース、毎フレームの確保なし)が各固定ステップ直前の位置を保存し、余り時間の比率`alpha = accumulator / FIXED_DT`で現在位置と補間した`float`配列を作る。`GameRenderer`は粒子の描画にその配列だけを使う(密度・測定円・HUD・星は無変更)。ラウンド開始時は`Reset()`で前状態を捨て、余りを捨てる分岐では`alpha=1`にして逆流を防ぐ。`GameManager.RENDER_INTERPOLATION_ENABLED`で無効化できる。**分担**: 純粋C#はローカルモデル(ornith-1.5-9b、2ラウンド)、配線はagy(1回)、テスト7件はClaude Code側で固定。EditMode 17/17。ユーザーの試遊(165Hz)で「滑らかになった気がする」として採用。

12. **際どさの可視化・音と振動・タイトルデモ(Unity版のみ、2026-09-18)**。unityroomのランキング(公開1週間、約68人)で、上位が猶予5ステップ(83ms)の中で離していること(9599点=192個)と、1粒子=50点で同点が多いことが分かった。ユーザーの決定: **スコア式・しきい値・ボードは変えない**(再設定が大変)、**ランダム性は足さない**(デイリーシード・揺動ありモードは却下)。入れたもの: (1) 離した直後に、離す前の状態を複製してカーソル固定で最大10秒進め(`MarginProbe`、毎フレーム30ステップずつ)、確定画面に「165 / 200 PARTICLES (LIMIT -5)」と「0.15s TO BANG」。しきい値を越えて離したときは「OVER THE EDGE 3/5 0.03s TO BANG」(金色)。(2) 確定音を際どさで変える(`playResolve(edgeRatio, graceWindow)`。輝き層は危険度の2乗、チャープときらめきは窓の中だけ)。Androidの振動(段階 15/25/40ms、確定 15〜70ms、爆発 160ms、`navigator.vibrate`)。(3) タイトル画面の右下に1/4サイズの小窓で自動デモ(専用の`BangSimulation`、シード固定、中央で押して猶予3ステップ目で離す。本編が始まると消える)。(4) 押している間、しきい値を越えたら猶予の進みを見せる。最初は密度バーの右の5目盛りにしたが、試遊で「ゲーム中にそこを見る余裕が無い」となり停止(`GameHud.SHOW_LIVE_EDGE_TICKS = false`)。ユーザー案で、カーソル中央の測定円の外周を5分割した弧(半径52、金色)に差し替えた。物理・乱数・golden基準は無変更、EditMode 31/31。分担: 純粋C#・配線・音・デモの4件をagy(gemini-3.8-flash-high)に並列委任、テスト12件と振動ブリッジはBrain。**unityroomへの反映と紹介文の差し替え(`docs/unityroom-description.md`)は未実施。**
