...
- Chrome拡張が未接続のため、**ユーザーがWebGLビルドを遊んで確認する**。JS版と並べて比べてもらう(2026-09-11にリポジトリを非公開にしてPagesが止まったので、JS版はリポジトリ直下を`python -m http.server`で配信して開く)。
...
3. **JS版は移植中は変更しない。**
4. **UIは「少し大きすぎるくらいで見える」ようにする(UI微調整の開始時)。** UIの大きさはJS版に合わせない。`GameHud.HudScale`(まず2)でHUD全体を拡大し、位置は最寄りの画面端か中央からの距離を広げるので、部品同士の並び方はJS版のまま。倍率はこの定数1つで調整する。ほかのUI微調整(BIG BANGの赤い光・💥・タイトルの字間)は行わない(ユーザー判断)。
5. **音量調節はUnity版だけに入れる。** ミュートボタンの左に常に見える横スライダー(ユーザーが選択)。JS版は凍結のまま。そのため**`unity/Assets/Plugins/WebGL/audio.jspre`はJS版`audio.js`のバイト単位のコピーではなくなった**。差分は`setVolume`/`getVolume`・音量の保存(`localStorage`の`bangs_edge_volume`)・`masterLevel()`(ミュートなら0、それ以外は音量の2乗)の追加と、マスター音量を設定する2行の置き換えだけ。`audio.js`を変えたときは`diff audio.js unity/Assets/Plugins/WebGL/audio.jspre`でこの差分だけが残ることを確かめて写す。スライダーを押すとミュートは解除され、ラウンドは始まらない。
6. **Unity版は揺動を止め(`WOBBLE_AX`/`WOBBLE_AY` 12/9→0)、`TAU_R`を3→8にする。JS版は以前のまま残す**(ユーザーが両方を試遊して判断)。理由は、現行の設定では「中央に置いて一定秒数で離す」で理論上の最高点の9割以上が取れ、危険度表示を読む意味が薄かったため(`docs/dev-notes.md`)。ユーザーの感想: 予想はしやすくなり、爆発の感覚を覚えて離すことを確実に学習できる。テンポと見た目は問題ない。一方で、以前のランダム性にも別の面白さがあった。**物理のロジック(`script.js`と`BangSimulation.cs`)は同じで、違うのは設定値だけ。** `tools/sim-harness/golden.js`は`GameConfig.cs`と`config.js`を突き合わせ、この3項目だけを読み込み時に上書きして基準データを作る(それ以外の項目で値が違うと止まる)。
7. **「射程内 N / 200 (必要 170)」は、射程内が満数(200)を下回ったときだけ表示する**(Unity版のみ)。盤面の中央付近(左右およそ±360px以内)では常に200で動かず、ユーザーから「変化していない」と報告があったため。到達不能の赤い警告は従来どおり。
8. **背景に重力レンズで歪む遠い星を置く(Unity版のみ、初版に入れる)**。ユーザーの案と選択: 歪みの中心はカーソル、強さは危険度(と押している時間の土台)で上がる / 歪み方は重力レンズ(点質量の主像、輪のように集まり明るくなる) / 星は粒子より小さく暗い青白で瞬き、画面の揺れに加わらない / 配置は起動ごとにランダム / 星ごとにゆっくり明滅 / 離した後は一度だけ逆向きに歪んでから元へ戻る(往復するバネは「変」だった) / 稀に流れ星が横切る。流れ星の軌道はまっすぐのまま、背景の星と同じレンズの写像で見た目だけ歪ませ、輪の外周に沿うように見せる(ユーザーの捉え方: ブラックホールは見え方を変えるだけで、進行方向は変えない)。実際の重力レンズに合わせ、流れ星には中心を挟んだ反対側に暗い二つ目の像も描く。実装は`unity/Assets/Scripts/Game/StarfieldRenderer.cs`で、物理と乱数には触れない(描画専用の`System.Random`)。ユーザーの試遊での評価(1回目): 粒子と見分けがつく、歪みで脅威度を認知しやすい、邪魔にならない、fpsは落ちない。(2回目): 押している時間の土台はあったほうが良い、明滅は目を凝らすと分かる程度で良い、流れ星の頻度・速さ・明るさは良い、重くならない。戻りのバネと、軌道を曲げる流れ星は上のとおり変更。
9. **unityroomのランキングを2つ入れる(Unity版のみ)**。ユーザーの選択: ボード1=スコア(大きいほど上位、`HighScoreDesc`、確定したラウンドの`FinalScore`、0点は送らない)、ボード2=**ビッグバンまでの最短時間**(小さいほど上位、`HighScoreAsc`、秒・小数点以下2桁)。送信はラウンドごとに自動(ライブラリが自己ベストを保持し、ボードごとに最短6秒間隔でまとめて送る)。爆発画面に「BANG TIME: 7.83s (BEST: 7.12s)」と、更新時は「★ NEW BEST TIME! ★」を出し、自己ベストは端末に保存(`bangs_edge_best_bang_time`)。**時間は押してから発火までの固定ステップ数×1/60秒**(`BangSimulation.RoundSteps`/`LastBangSteps`)で数えるので、fpsやタブの一時停止に左右されない。物理と乱数は変えていない。ライブラリは公式の`com.unityroom.client`(v0.9.6、gitのURLで固定)。シーンを編集しないので、`UnityroomApiClient`は非アクティブのGameObjectに追加し、非公開フィールド`HmacKey`にリフレクションでキーを入れてから有効にする。**HMACキーは`unity/Assets/Resources/Secrets/unityroom-hmac.txt`に置き、`.gitignore`で除外**(キーはビルドには入るが、将来リポジトリを公開してもgitには残らない)。キーが無ければ送信しない(警告ログのみ)。
