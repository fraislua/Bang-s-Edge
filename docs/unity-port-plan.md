...
| HUD(密度バー・段階表示・射程内の個数と警告・fps・SCORE/HIGH・待機/確定/ビッグバン画面) | uGUIをコードで組み立て、フォントはUnity内蔵の`LegacyRuntime.ttf` | 日本語の2文と絵文字の扱いは §5-2 |
| ミュートボタン(右上28px、押してもラウンドを始めない) | 同じ位置・同じ判定 | |
| `audio.js`(Web Audioの合成音: ドローン7ノード・警告パルス・爆発3層・確定和音) | **§5-1で決定** | |
...
| P5 | HUDとミュートボタン | agy / gemini-3.8-flash-high | 同上 |
| P6 | 音(§5-1の方式による) | agy(方式Aならjslib、方式BならC#) | ユーザーが聴いて比較 |
...
1. **音の方式**
   - **A. `audio.js`をそのまま使う(jslibで呼ぶ)**: Web Audioのコードを流用するので音はJS版と同じ。WebGLビルドでしか鳴らず、Unityエディタ上では無音になる。
   - **B. C#で作り直す**: 事前に生成した音声クリップの音量・ピッチを変える方式になり、危険度に連動して音色が変わるドローン(フィルタとLFOの変化)は再現できない。音は変わる。
2. **HUDの日本語2文(「射程内 N / 200 (必要 170)」「この位置では到達不能 — 画面の中央へ」)と絵文字(💥・★)**: WebGLでは、同梱したフォントに含まれる文字しか表示されない。英語にするか、日本語フォント(ライセンスがOFLのものなど)を同梱するか。
3. **JS版を移植中は変更しない**方針でよいか。
